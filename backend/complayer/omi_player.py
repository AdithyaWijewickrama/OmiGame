...
from backend.card_pack import get_biggest_rank, get_bigger_than, Card, Hand, get_lowest_rank, sum_of_ranks, \
    empty_suits_pack
from backend.complayer.omi_game import get_next_player_relative_to
...
class Player:
    from backend.complayer.omi_game import OmiGame, Team

    def __init__(self, name: str, game: OmiGame, team: Team):
        self.cards = empty_suits_pack()
        self.game = game
        self.id = get_next_player_id()
        self.name = name
    ...
    def get_next(self, hand: Hand):
        ...
        if len(list(self.cards.values())) == 1:
            ...
            self.think("No leading suit in hand...")
            a = {}
            ...
            if not cut:
                ...
                if my_turn == 2:
                    ...
                    if current_owner == self.player_op:
                        self.think("Current owner is opponent...")
                        if (not self.game.players()[get_next_player_relative_to(self.id)].seen_empty_suit[lead_suit]
                ...
                if self.have_suit(self.game.trump_suit):
                    if my_turn == 3 or my_turn == 4:
                        ...
                        if self.game.players()[
                            get_next_player_relative_to(self.id)
                        ].seen_empty_suit[
